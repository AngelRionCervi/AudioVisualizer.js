class CircularType {
    ...
    getAvg(arr) {
        ...
        return (total / arr.length) * this.circularType.innerCircleMult;
    }
    ...
    renderCircular() {
        ...
        if (this.circularType.skipNull) {
            freqs = new Uint8Array(freqs.filter(el => el != 0));
        } else {
            freqs = new Uint8Array(freqs);
        }

        let radius = this.circularType.innerCircleReact ? this.circularType.radius + this.getAvg(freqs) : this.circularType.radius;
        radius += this.radInc;
